...
|---|---|---|---|---|
| **F6** | **Découper `vibed/src/mcp.rs`** en modules `tools/{fs,memory,svc,sectools}.rs`. **En cours (2026-07-14)** : `tools/svc.rs` ✅ + `tools/sectools.rs` ✅ extraits (mcp.rs 4257 → 3744 lignes). **Reste : `fs` et `memory`** | Revue adversariale Fable 5 (finding F6, 7/7 traités sauf celui-ci) | **~0,5–1 j** restant (fs + memory). Ces deux familles sont **plus couplées** : elles partagent avec le câblage `mcp.rs` `builtin_denied` (fs — aussi utilisé par `handle_tools_call` et `policy.check`), `MEMORY_DIR` / `utc_date_string` / `utc_iso8601` (memory — aussi utilisés par `agent.thinking`/`agent.sessions`/vibectl), et des helpers entrelacés qui doivent **rester** dans `mcp.rs`. L'extraction sûre = threading de ~10 symboles partagés en `pub(crate)` + déplacement des tests, à faire d'un bloc pour garder « zéro changement » vérifiable | Refactor **mécanique sans gain fonctionnel**. svc/sectools (autonomes) faits ce soir ; fs/memory (couplés) **délibérément différés à une session dédiée** — les extraire en fin de créneau risquerait un changement de comportement subtil sur des surfaces sécurité (denylist, confinement home). Idéalement juste après le merge de PR #11 |

**F6 — critère de « fait »** : `mcp.rs` réduit à un module de câblage, chaque famille d'outils dans son propre fichier `tools/*.rs`, **zéro changement de comportement** (les 147 tests passent sans modification de leurs assertions), `clippy -D warnings` et `fmt --check` verts. **État 2026-07-14** : svc + sectools ✅ ; fs + memory à faire.
