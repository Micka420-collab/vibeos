...
### 🌱 Born blank — Genesis
The OS image contains **no factory memory**. On first boot, `vibeos-genesis.service` (guarded by `ConditionPathExists=!/var/lib/vibeos/memory/.initialized`) runs `/usr/libexec/vibeos/genesis.sh` and builds the machine's memory from scratch under `/var/lib/vibeos/memory` — **in cleartext in v0.1**; LUKS/TPM2 volume encryption is a **Phase 3** deliverable. At birth, **the AI chooses its own character** (`personality.toml`: name, archetype, 6 traits, tone) — **unique per install** and deterministic (derived from `machine_id`, offline), with an **awakening** printed to the console ([ADR-029](docs/DECISIONS.md)). **Amnesic mode** (inspired by Tails) recreates that memory in tmpfs **on every boot** — nothing survives shutdown: the **systemd generator** is **delivered** (enabled by the `vibeos.amnesic=1` kernel parameter); its VM validation remains **Phase 3**. Full spec in [docs/MEMORY.md](docs/MEMORY.md).

### 🤖 AI as a citizen of the OS — vibed + MCP + policies
> **Status:** the `vibed` binary is **embedded in the image** (compiled multi-stage in `os/Containerfile`, installed at `/usr/bin/vibed`). At boot, `vibed.service` (preset-enabled) starts, **loads and enforces the installed policy** (`/etc/vibeos/policy.d/`, fail-closed), **serves the MCP server** on `/run/vibed/mcp.sock` and **audits** every call under `/var/lib/vibeos`. On the client side, the image ships the **Claude Code MCP config** (`/etc/skel/.claude.json`): the `vibeos` server is discovered **with no manual setup** (prerequisite: the `vibeos-agents` group). The **per-tool sandbox mechanism** ([ADR-019](docs/DECISIONS.md)) is delivered — hardened transient systemd units + the low-privilege `vibed-tool` helper, exercised by the `sandbox.probe` (T1) tool which judges the confinement actually obtained — but its **proof on real hardware remains to be done** (machine-gated); also still to come: dedicated SELinux `vibed_t`, `User=vibed`, and external TPM/Rekor anchoring of the audit chain (**Phase 4**). The `vibed` crate is tested (471 green tests, including 14 end-to-end MCP integration tests over a socket + 3 policy tests); the audit log is **SHA-256 hash-chained** (`vibed --verify-audit`). The agent supervisor now **counts and bounds tokens** (`frugale`/`équilibrée`/`performance` modes, a `--tokens` budget, cache efficiency logged — [docs/TOKENS.md](docs/TOKENS.md)), and `memory.query` gains a **recall ranking** (opt-in `rank` mode) and **knowledge consolidation** (dedup of `facts.jsonl` via `fold`/`vibectl memory knowledge`) — never raising a fact's confidence ([ADR-030](docs/DECISIONS.md)). **The VibeOS identity is now active**: `/usr/lib/os-release` is rewritten (`ID=vibeos` — no more "Fedora" in the boot menu, at login, or in `fastfetch`), the VibeOS **Plymouth** splash and **SDDM** greeter are set as the build-time defaults (initramfs regenerated, verified), and the logo ships; the **first boot** on the reference PC is unblocked by the NVIDIA kernel arguments (`nvidia-drm.modeset=1` + `nouveau` blacklist, set via `bootc kargs.d` **and** the kickstart) — ⚠️ **Secure Boot must stay disabled** until our kmods are MOK-signed (**Phase 4**, [docs/HARDWARE.md](docs/HARDWARE.md)). The AI citizen **knows itself**: `agent.identity` (T0) and `vibectl whoami` publish its birth character (name/archetype/traits, [ADR-029](docs/DECISIONS.md)) **and the operating style** derived from it — conduct, never a permission: the floor (tier, human approval, denylist, audit) is decided elsewhere and never consults the style — while the greeter **names** it at login. And it **looks itself in the eye**: `vibectl conscience` confronts the character it DECLARES with the conduct the audit log has RECORDED (denial rate against declared caution, tool diversity against curiosity, proposals against initiative) and says so itself when the two diverge — observation only, never a permission, and an axis with no data answers "no data" rather than flattering. Recent hardening: four **clock fail-safes** (a backward clock — NTP, VM snapshot, unset RTC — can no longer reactivate open mode, corrupt the audit chain, or revive an approval grant) and one sandbox **confinement escape** (an unfiltered `:` in `BindReadOnlyPaths` that re-exposed every home) are fixed, each with a regression test.
...
| **Birth interview** — opt-in script `genesis-interview.sh`: 4 questions (name, language, domain, collaboration style) → **append-only** `user/` memory, escaped (anti-poisoning), idempotent and **never blocking at boot** (no-op without a TTY); called by `genesis.sh` after the awakening | ✅ Delivered (opt-in); rich interactive version = Phase 3 |
| **`vibectl memory reset`** — memory factory reset (root-only, `--yes` guard: explicit refusal listing what would be destroyed), purges the birth files + subdirectory contents then re-arms Genesis (LUKS crypto-erase follows in Phase 3) | ✅ Delivered |
| **Client-side cosign verification** — keyless sigstore policy **delivered STAGED** (`policy-strict.json`, pinned anchors) + a skopeo rejection proof in CI; activation machine-gated (c/image cannot match URI SANs yet) | ✅ Mechanism delivered (staged); activation = machine-gated |
...
| **`agent.activity` (T0)** — the citizen re-reads its **own acts** (recent governed calls, refusals included), confined per uid | ✅ Delivered ([ADR-026](docs/DECISIONS.md)) |
| **`user.model` (T0)** — **derived & transparent** model of *how you work* (preferences, patterns, rhythm, friction) + deterministic **anticipation** of your next actions; derived from already-governed data, confined per uid, local/erasable | ✅ Foundation delivered ([ADR-028](docs/DECISIONS.md)); opt-in learning/embeddings = to come |
| **Living Genesis** — at first boot the AI **chooses its character** (`personality.toml`: name, archetype, 6 traits, tone), **unique per install** and deterministic (derived from `machine_id`, offline), with a futuristic **awakening** on the console; `personality` scope (`memory.query`) + `ai-personality` selfcheck invariant. The character **bends toward the human** (`[adaptation]` table ← `user.model`) | ✅ Birth + awakening delivered ([ADR-029](docs/DECISIONS.md)); living adaptation loop & graphical HUD ceremony = to come/machine-gated |
...
| **Build** | Green CI (native runners, ~15 min/arch) · `bootc container lint` OK · 471 green `vibed` tests (+ 27 Zed extension tests + 100 HUD client checks) |
