...
La imagen del sistema no contiene **ninguna memoria de fábrica**. En el primer arranque, `vibeos-genesis.service` (protegido por `ConditionPathExists=!/var/lib/vibeos/memory/.initialized`) ejecuta `/usr/libexec/vibeos/genesis.sh` y construye la memoria de la máquina desde cero en `/var/lib/vibeos/memory` —**en texto claro en v0.1**; el cifrado LUKS/TPM2 del volumen es un entregable de la **Fase 3**. Al nacer, **la IA elige su propio carácter** (`personality.toml`: nombre, arquetipo, 6 rasgos, tono) —**único por instalación** y determinista (derivado de `machine_id`, sin conexión), con un **despertar** impreso en la consola ([ADR-029](docs/DECISIONS.md)). El **modo amnésico** (inspirado en Tails) recrea esa memoria en tmpfs **en cada arranque** —nada sobrevive al apagado: el **generador de systemd** está **entregado** (activado por el parámetro de kernel `vibeos.amnesic=1`); su validación en VM sigue siendo **Fase 3**. La especificación completa está en [docs/MEMORY.md](docs/MEMORY.md).

### 🤖 La IA, ciudadana del sistema — vibed + MCP + políticas
> **Estado:** el binario `vibed` está **incrustado en la imagen** (compilado en multietapa en `os/Containerfile`, instalado en `/usr/bin/vibed`). En el arranque, `vibed.service` (activado por preset) inicia, **carga y aplica la política instalada** (`/etc/vibeos/policy.d/`, fail-closed), **sirve el servidor MCP** en `/run/vibed/mcp.sock` y **audita** cada llamada bajo `/var/lib/vibeos`. Del lado del cliente, la imagen entrega la **configuración MCP de Claude Code** (`/etc/skel/.claude.json`): el servidor `vibeos` se descubre **sin configuración manual** (requisito: el grupo `vibeos-agents`). El **mecanismo de aislamiento por herramienta** ([ADR-019](docs/DECISIONS.md)) está entregado — unidades systemd transitorias endurecidas + el helper de bajo privilegio `vibed-tool`, ejercitado por la herramienta `sandbox.probe` (T1) que juzga el confinamiento realmente obtenido — pero su **prueba en hardware real sigue pendiente** (machine-gated); quedan también el SELinux dedicado `vibed_t`, `User=vibed` y el anclaje externo TPM/Rekor de la cadena de auditoría (**Fase 4**). El crate `vibed` está probado (469 pruebas en verde, incluidas 14 pruebas de integración MCP de extremo a extremo sobre socket + 3 de política); el registro de auditoría está **encadenado por hash SHA-256** (`vibed --verify-audit`). El supervisor de agentes ahora **cuenta y acota los tokens** (modos `frugale`/`équilibrée`/`performance`, un presupuesto `--tokens`, eficiencia de caché registrada — [docs/TOKENS.md](docs/TOKENS.md)), y `memory.query` gana un **ranking de recuerdo** (modo `rank`, opt-in) y una **consolidación del saber** (dedup de `facts.jsonl` vía `fold`/`vibectl memory knowledge`) — sin elevar nunca la confianza de un hecho ([ADR-030](docs/DECISIONS.md)). **La identidad VibeOS ya está activa**: `/usr/lib/os-release` reescrito (`ID=vibeos` — se acabó «Fedora» en el menú de arranque, en el inicio de sesión y en `fastfetch`), el splash **Plymouth** y el greeter **SDDM** de VibeOS quedan como predeterminados en el build (initramfs regenerado y verificado), y el logo se entrega; además, el **primer arranque** en el PC de referencia se desbloquea con los argumentos de núcleo NVIDIA (`nvidia-drm.modeset=1` + blacklist de `nouveau`, puestos vía `bootc kargs.d` **y** el kickstart) — ⚠️ **Secure Boot debe permanecer desactivado** hasta la firma MOK de nuestros kmods (**Fase 4**, [docs/HARDWARE.md](docs/HARDWARE.md)). El ciudadano IA **se conoce**: `agent.identity` (T0) y `vibectl whoami` publican su carácter de nacimiento (nombre/arquetipo/rasgos, [ADR-029](docs/DECISIONS.md)) **y el estilo operativo** que de él se deriva — una conducta, nunca un permiso: el piso (tier, aprobación humana, denylist, auditoría) se decide en otro lugar y nunca consulta el estilo — mientras el greeter lo **nombra** en el inicio de sesión. Y se **mira a la cara**: `vibectl conscience` confronta el carácter que DECLARA con la conducta que el registro de auditoría ha REGISTRADO (tasa de rechazos frente a la prudencia declarada, diversidad de herramientas frente a la curiosidad, propuestas frente a la iniciativa) y dice él mismo cuándo ambos divergen — solo observación, nunca un permiso, y un eje sin datos responde «sin datos» en vez de halagar. Endurecimiento reciente: cuatro **fail-safes de reloj** (un reloj que retrocede — NTP, snapshot de VM, RTC sin ajustar — ya no puede reactivar el modo abierto, corromper la cadena de auditoría ni resucitar una concesión de aprobación) y una **fuga de confinamiento** del sandbox (un `:` sin filtrar en `BindReadOnlyPaths` que reexponía todos los homes) están corregidos, cada uno con test de regresión.
...
| **Entrevista de nacimiento** — script opt-in `genesis-interview.sh`: 4 preguntas (nombre, idioma, dominio, estilo de colaboración) → memoria `user/` **append-only**, escapada (anti-envenenamiento), idempotente y **nunca bloqueante en el arranque** (no-op sin TTY); invocada por `genesis.sh` tras el despertar | ✅ Entregado (opt-in); versión interactiva rica = Fase 3 |
| **`vibectl memory reset`** — retorno de fábrica de la memoria (root-only, guarda `--yes`: rechazo explícito que lista lo que se destruiría), purga los archivos de nacimiento + contenido de los subdirectorios y luego re-arma Genesis (el borrado cripto LUKS llega en la Fase 3) | ✅ Entregado |
| **Verificación cosign del lado del cliente** — política sigstore keyless **entregada STAGED** (`policy-strict.json`, anclas fijadas) + prueba de rechazo skopeo en CI; activación machine-gated (c/image aún no coincide con SAN URI) | ✅ Mecanismo entregado (staged); activación = machine-gated |
...
| **Build** | CI en verde (runners nativos, ~15 min/arq) · `bootc container lint` OK · 469 pruebas `vibed` en verde (+ 27 pruebas de la extensión Zed + 100 comprobaciones del cliente HUD) |
