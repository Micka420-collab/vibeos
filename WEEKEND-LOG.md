...
### 2026-07-19 (dimanche, suite) — le ecommerce rendu concret + le maillon sécu-avant-prod

Tu m'as dit « continue » : j'ai fermé les deux derniers vrais maillons du parcours, plutôt que d'empiler du remplissage.

- **[#127, mergée] `ECOMMERCE.md` — l'ecommerce de A à Z, concret.** Le substrat *servait* l'ecommerce (base, stockage objet pour les images, Meilisearch pour la recherche) mais le chemin n'était que **mentionné**. Le runbook monte une vraie boutique **Medusa** (MIT, v2) branchée sur les 3 modèles compose livrés. Ancré sur les faits Medusa v2 vérifiés à la source (agent Fable 5), honnête sur les pièges **réels** — le plus vicieux : le provider S3 de Medusa (AWS SDK v3) envoie des checksums CRC32 qui **cassent les uploads SeaweedFS** → `requestChecksumCalculation: WHEN_REQUIRED` (sans ça, images produit corrompues). Rattrapé *avant* que tu tombes dessus.
- **[#128, en build, durcie Fable 5] `PRODUCTION.md §1-bis` — audite avant d'exposer.** VibeOS est security-first et livre une trousse cybersécu (nmap, lynis…) mais le parcours prod ne disait pas de **s'en servir sur sa propre infra**. Le §1 *déclarait* le pare-feu sans le **prouver**. §1-bis relie les deux trousses : `nmap` **depuis l'extérieur** pour vérifier empiriquement que seuls 22/80/443 répondent, `lynis` pour le durcissement de l'hôte, frontière de gouvernance défensif/offensif posée. **Revue Fable 5 → 3 corrections** dont un vrai vecteur de faux sentiment de sécurité : je présentais `closed` comme un succès de scan, or `closed` = le pare-feu a **laissé passer** le paquet (RST) — seul `filtered` prouve le filtrage. + trou IPv6 (le §2 impose un AAAA → surface v6 que le scan v4-only ignorait).

**La synthèse.** La mission — *« faire des SaaS et du ecommerce de A à Z, les mettre en prod, outils d'analyse de perf »* — est **couverte de bout en bout, des deux côtés** : QUICKSTART (SaaS générique), ECOMMERCE (boutique Medusa), PRODUCTION (self-hosted durci, avec gate sécu), sur 6 modèles serveur vérifiés E2E + testés en CI, plus les deux modes de prod (managé et self-hosted). J'ai **maximisé la surface sûre-et-autonome** ; je n'invente pas de remplissage au-delà. Le gros du travail restant — **implémenter** `deploy.*`/`browser.*` — reste sur **ta décision ADR-019** (la clé de voûte, décision-ready dans #120/#124/#126). PR ouvertes flaggées pour toi : #120, #124, #126, #128.

### 2026-07-20 — « go ADR-019 » : je construis le socle, et tu me dis de décider avec Fable 5

Tu as tranché : **« go ADR-019 »**, puis **« prends les décisions avec Fable 5… but expérimental : l'OS pour les agents IA, qui font tout comme un vrai utilisateur »**. Compris — je ne te renvoie plus les décisions, je tranche avec Fable 5 en contradicteur, et je garde l'invariant : *tout faire comme un vrai utilisateur se réalise **à travers** la gouvernance (tiers + sandbox + audit), pas en la retirant.* J'ai canonisé ADR-019 (#126 mergé) et j'implémente le **socle commun à `deploy.*` et `browser.*`**, par petits incréments testés, chacun relu Fable 5 :

- **[#129, mergé] Incrément 1 — profils de durcissement (pur).** Un module qui compile en Rust *testable* les propriétés d'unité systemd transitoire par classe d'outil. La revue Fable 5 a rattrapé du **lourd** avant tout spawn : `MemoryDenyWriteExecute` cassait vercel/railway (Node/JIT V8) → W^X par-CLI ; bug de concurrence (HOME fixe → l'isolation d'uid saute) → nonce par invocation ; DNS bloqué, workspace masqué, trous ADR-022 (pas d'`IPAccounting`, loopback entier ouvert). 10 invariants testés.
- **[#130, en revue] Incrément 2 — argv `systemd-run` + helper `vibed-tool` (probe).** Le helper de faible privilège qui, sur cible, **prouve** que le sandbox s'est appliqué. Revue Fable 5 **empirique sur systemd 255** : le canal IPC (`--pipe`) est validé ; mais ma « preuve » v1 était FAUSSE (les inodes de namespace sont identiques à l'hôte sous `RestrictNamespaces` — il empêche de *créer* des ns, pas de *déplacer* le service). Refondu : preuve **comportementale** (`unshare` → EPERM sous deploy) + champs `/proc/self/status` (Seccomp, CapBnd, NoNewPrivs).
- **[#131, en revue] Incrément 3 — `spawn_transient` réel.** `vibed` exec `systemd-run` (pas l'outil), payload de contrôle sur stdin depuis un thread (sans deadlock), stdout=résultat / stderr=diagnostics séparés. Testé avec un faux `systemd-run`, discipline `svc.rs`.

**Règle de merge que je tiens** : ces incréments sont **inerts** (rien ne les appelle, aucune capacité d'exécution) et ne touchent **pas** `policy`/`approval`/`audit` → auto-mergeables une fois verts + relus Fable 5. Le prochain incrément **câble un vrai outil** (`sandbox.probe`) dans le dispatch MCP : c'est là que la capacité d'exécution apparaît → revue renforcée, et les changements de `policy.rs` restent flaggés pour toi. Le vrai comportement systemd (le sandbox, le userns de Chromium, SELinux enforcing) ne se valide que **sur ton boot** — le probe est fait pour ça.

### 2026-07-20 (nuit, suite) — implémentation ADR-019 COMPLÈTE + le handoff de vérification

La nuit a bouclé le socle. Tout est **mergé sur `main`**, chaque maillon relu Fable 5 :

- **[#133, mergé] Incrément 4 — `sandbox.probe` (T1), le premier outil gouverné vivant.** Il pilote toute la chaîne (compile un profil → spawne le service transitoire → capture le rapport du helper → **note** le confinement) et rend `confined: true/false` + les écarts. La revue Fable 5 renforcée a rattrapé un **bloquant** : l'outil était **mort à la livraison** — aucune règle de politique, donc le default-deny absolu le refusait. Corrigé (règle `sandbox-probe` T1 + test de parité). Tier T1 **tranché avec Fable 5** (rayon < `fs.write`, borné par le rate-limiter), à deux conditions gravées : binaire/argv/stdin fixes ; le rapport n'est de confiance que parce que le helper est un binaire de confiance de l'image. + `assess()` renforcé (comparaison `ns_mnt`, euid/cap_eff/umask, nonce anti-collision).
- **[#134, mergé] `verify-sandbox` — le handoff on-target.** Le socle est testé côté daemon, mais **que systemd/SELinux/le noyau confinent vraiment ne s'observe que sur cible**. Ce script (client MCP léger, stdlib Python) appelle `sandbox.probe` au boot et rend PASS/FAIL. La revue Fable 5 a corrigé un vrai piège : un outil de vérif ne doit **jamais mentir sur la raison** de l'échec → statut ternaire (confiné / a-tourné-mais-pas-confiné / **pas-pu-vérifier**) avec les bons codes 0/1/2. Validé **e2e** contre un `vibed` dev.

**Où on en est, honnêtement.** L'**implémentation ADR-019 est complète et vérifiable** : profils durcis, `spawn_transient` à capture bornée, helper `probe`, l'outil `sandbox.probe`, et `verify-sandbox`. **Huit revues Fable 5** cette nuit, chacune a rattrapé du réel. J'ai poussé et vérifié **tout le sûr-et-autonome** ; je n'ai PAS fait de chirurgie de moteur prématurée (`[rule.deploy]` pour un outil qui n'existe pas encore).

**Ton prochain pas, en une commande** — au boot : `/usr/libexec/vibeos/verify-sandbox`. S'il rend « CONFINES », le sandbox confine vraiment sur ta machine, et je branche `deploy.plan`/`browser.navigate` sur cette **chaîne prouvée**. La suite attend **tes décisions** (je ne les grave pas seul) : tes **cibles deploy réelles** (Fly/Vercel), `chromium-headless` **~300 Mo dans l'image**, et le **module SELinux** du navigateur — tous validables seulement sur ton boot. PR de design encore ouvertes pour ta lecture : #120 (deploy), #124 (browser).

### 2026-07-20 (nuit, tard) — le build de `main` réparé (deux pannes infra, zéro dette de code)

Après les merges ADR-019, le build-os de `main` est tombé **rouge deux fois** — mais **aucune cause n'était mon code** (vibed compile+teste, 206 tests ; l'arbre identique passait en PR). J'ai diagnostiqué et corrigé les deux, en pleine instabilité réseau nocturne (503 GitHub, miroirs Fedora qui coupent) :

1. **Digest de base F44 purgé** (`reading manifest sha256:892ab960… : manifest unknown`). Quay purge les vieux index de la base *vivante* :44 ~chaque jour ; le pin a cessé de résoudre entre le build de l'incrément 3 (vert) et les merges suivants. → **bumpé** `892ab960→4ff9e301` (skopeo, multi-arch, garde de fraîcheur verte). La fragilité que l'auto-bump #112/#114 gère, faite à la main pour débloquer vite.
2. **`kernel-devel` en retard sur le kernel du base** (`No match for argument: kernel-devel-7.1.4-200.fc44`). Le nouveau base embarque le kernel 7.1.4-200 ; les dépôts stables laggent à 7.1.3-201 ; l'akmod NVIDIA ne peut pas builder sans le `kernel-devel` **exact**. → **repli sur koji par NVRA** (koji a tout build ; `kernel-devel-7.1.4-200` vérifié présent, HTTP 200). Fix robuste (patron ublue) qui **sécurise TOUS les futurs bumps** face à ce décalage kernel/kernel-devel.

Les deux dans **[#136, mergé]**. **Build-os de `main` sur le merge (`f6d3e9b`) = ✅ success.** → l'implémentation **ADR-019 est complète, mergée ET buildable en image**. Le volet autonome est **clos proprement** ; il ne reste que **ton boot + tes décisions gated** (verify-sandbox, cibles deploy, chromium image, SELinux).
