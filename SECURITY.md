...
- Le journal d'audit est un fichier **JSONL append-only chaîné par hachage** : `/var/lib/vibeos/audit/vibed.jsonl`. Il est en **lecture interdite et écriture interdite pour les agents**, à la fois par la politique livrée (voir `security/policy.d/default.toml`) et par une denylist codée en dur dans `vibed` (indépendante de la politique).
