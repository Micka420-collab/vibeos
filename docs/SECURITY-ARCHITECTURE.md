...
2. ✅ **Fait** : `fs_read` vérifie le **propriétaire de l'inode ouvert** pour un scope `Home` (`opened.uid() == caller.uid` sur le fd, fs.rs) — défense hardlink cross-owner : un hardlink vers l'inode d'un autre utilisateur ou de root est refusé **même sans** `fs.protected_hardlinks` ; les lectures system (owned root) en sont exemptées. Et le garde `home_is_too_broad` **rejette désormais tous les ancêtres larges** (`/`, `/var`, `/var/home`, `/home`, `/usr`, `/etc`, …), pas seulement `/` — un `/etc/passwd` mal configuré (home = `/var`) ne peut plus rendre `is_within` trivialement large ([PR #24](https://github.com/Micka420-collab/vibeos/pull/24), testé `home_is_too_broad_rejects_ancestors_but_accepts_real_homes`). Résidu **sans objet** : `fs_list` n'a pas le contrôle propriétaire par-fichier, mais il *liste* (ne lit aucun contenu) et un répertoire ne se hardlinke pas — la cible reste confinée par `confine_read`.
...
**JSONL append-only, rotation par jour UTC** : répertoire `/var/lib/vibeos/audit/`, un fichier `vibed-AAAA-MM-JJ.jsonl` par jour (aucun fichier ne croît sans borne — sinon disque plein + fail-closed finiraient par bloquer tout appel d'outil). Un objet JSON par appel d'outil : horodatage, **identité de l'appelant** (uid/gid/pid capturés par peer credentials sur le socket), outil, **digest FNV-1a des arguments** (non cryptographique — corrélation, pas preuve d'intégrité), règle de politique appliquée, tier, décision, résultat d'exécution. Les fichiers sont ouverts en `O_APPEND` par `vibed` uniquement, `fsync` à chaque enregistrement, et **illisibles/inaccessibles en écriture pour les agents** à double titre : chemins refusés de la politique ([../security/policy.d/default.toml](../security/policy.d/default.toml)) **et** denylist intégrée au code de `vibed` (`/var/lib/vibeos/audit/**`), qui s'applique quelle que soit la politique. Si l'écriture d'audit échoue, l'action échoue (fail-closed sur le chemin Allow).
