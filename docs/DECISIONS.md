...
- Résiduel assumé : le câblage de `recall.rs` dans `memory.query`, la **production des vecteurs** (ollama local), la **consolidation** journal→`knowledge` et la lecture d'une politique de poids sont **à venir** ; ce qui est livré est la fondation mesurée, bornée et pure — honnête sur son gating.
