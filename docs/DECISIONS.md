...
- Câblage **livré** : `memory.query` mode `rank: true` (opt-in, `journal`/`knowledge`) classe les événements par le score de rappel (moteur lexical). Résiduel assumé : la **production des vecteurs** (ollama local) et le branchement du moteur **vectoriel**, la **consolidation** journal→`knowledge` et la lecture d'une politique de poids sont **à venir** ; ce qui est livré est mesuré, borné, additif et testé — honnête sur son gating.
