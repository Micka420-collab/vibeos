...
> **Status:** Das `vibed`-Binary ist **im Image eingebettet** (mehrstufig in `os/Containerfile` kompiliert, installiert unter `/usr/bin/vibed`). Beim Start startet `vibed.service` (per Preset aktiviert), **lädt und erzwingt die installierte Policy** (`/etc/vibeos/policy.d/`, fail-closed), **bedient den MCP-Server** unter `/run/vibed/mcp.sock` und **auditiert** jeden Aufruf unter `/var/lib/vibeos`. Auf Client-Seite liefert das Image die **MCP-Konfiguration von Claude Code** (`/etc/skel/.claude.json`): Der `vibeos`-Server wird **ohne manuelle Konfiguration** entdeckt (Voraussetzung: die Gruppe `vibeos-agents`). Der **Sandbox-Mechanismus pro Werkzeug** ([ADR-019](docs/DECISIONS.md)) ist ausgeliefert — gehärtete transiente systemd-Units + der niedrig privilegierte Helper `vibed-tool`, ausgeübt durch das Werkzeug `sandbox.probe` (T1), das die tatsächlich erreichte Einschließung beurteilt — sein **Nachweis auf echter Hardware steht aber noch aus** (machine-gated); ebenfalls noch ausstehend: dediziertes SELinux `vibed_t`, `User=vibed` und die externe TPM/Rekor-Verankerung der Audit-Kette (**Phase 4**). Das `vibed`-Crate ist getestet (466 grüne Tests, darunter 12 End-to-End-MCP-Integrationstests über Socket + 3 Policy-Tests); das Audit-Log ist **per SHA-256-Hash verkettet** (`vibed --verify-audit`). Der Agenten-Supervisor **zählt und begrenzt jetzt Tokens** (Modi `frugale`/`équilibrée`/`performance`, ein `--tokens`-Budget, Cache-Effizienz protokolliert — [docs/TOKENS.md](docs/TOKENS.md)), und `memory.query` erhält ein **Recall-Ranking** (opt-in `rank`-Modus) sowie eine **Wissenskonsolidierung** (Dedup von `facts.jsonl` via `fold`/`vibectl memory knowledge`) — ohne je die Konfidenz eines Fakts zu erhöhen ([ADR-030](docs/DECISIONS.md)). **Die VibeOS-Identität ist jetzt aktiv**: `/usr/lib/os-release` wurde neu geschrieben (`ID=vibeos` — kein „Fedora“ mehr im Boot-Menü, beim Login oder in `fastfetch`), der VibeOS-**Plymouth**-Splash und der **SDDM**-Greeter sind zur Build-Zeit als Standard gesetzt (initramfs neu erzeugt und verifiziert), und das Logo wird ausgeliefert; der **erste Boot** auf dem Referenz-PC wird durch die NVIDIA-Kernel-Argumente entsperrt (`nvidia-drm.modeset=1` + `nouveau`-Blacklist, gesetzt über `bootc kargs.d` **und** den Kickstart) — ⚠️ **Secure Boot muss deaktiviert bleiben**, bis unsere kmods MOK-signiert sind (**Phase 4**, [docs/HARDWARE.md](docs/HARDWARE.md)). Der KI-Bürger **kennt sich selbst**: `agent.identity` (T0) und `vibectl whoami` veröffentlichen seinen Geburtscharakter (Name/Archetyp/Eigenschaften, [ADR-029](docs/DECISIONS.md)) **und den daraus abgeleiteten Arbeitsstil** — ein Verhalten, niemals eine Erlaubnis: die Untergrenze (Tier, menschliche Freigabe, Denylist, Audit) wird anderswo entschieden und konsultiert den Stil nie — während der Greeter ihn beim Login **benennt**. Und er **sieht sich selbst ins Gesicht**: `vibectl conscience` stellt den Charakter, den er ERKLÄRT, dem Verhalten gegenüber, das das Audit-Log AUFGEZEICHNET hat (Ablehnungsquote gegen erklärte Vorsicht, Werkzeugvielfalt gegen Neugier, Vorschläge gegen Initiative) und sagt selbst, wenn beide auseinandergehen — reine Beobachtung, nie eine Erlaubnis, und eine Achse ohne Daten antwortet „keine Daten“, statt zu schmeicheln. Aktuelle Härtung: vier **Uhren-Fail-Safes** (eine zurücklaufende Uhr — NTP, VM-Snapshot, ungestellte RTC — kann den offenen Modus nicht mehr reaktivieren, die Audit-Kette nicht korrumpieren und keine Freigabe wiederbeleben) sowie ein **Confinement-Ausbruch** der Sandbox (ein ungefiltertes `:` in `BindReadOnlyPaths`, das alle Home-Verzeichnisse wieder freilegte) sind behoben, jeweils mit Regressionstest.
...
| **Build** | Grüne CI (native Runner, ~15 min/Arch) · `bootc container lint` OK · 466 grüne `vibed`-Tests (+ 27 Tests der Zed-Erweiterung + 100 Prüfungen des HUD-Clients) |
