...
| `check-threatmodel-tools.py` | un outil du catalogue MCP (`build_tool_catalog`) sans ligne dans `docs/THREAT-MODEL.md` §6 | la règle « chaque outil ajoute sa ligne » était documentée mais non exécutée — la moitié du catalogue avait dérivé sans ligne |
