...
- **T2+ = approbation humaine, toujours.** `pkg.install` et `svc.restart` créent une **requête d'approbation** (l'agent reçoit un `isError` avec un id) tracée dans le journal d'audit `/var/lib/vibeos/audit/` (fichier daté, avec l'identité de l'appelant uid/gid/pid). L'opérateur exécute `vibectl approve <id>` ; au ré-appel identique, `vibed` consomme le grant à usage unique et exécute. Le dialogue d'approbation Plasma/HUD et les backends T2 réels (`pkg.install`/`svc.restart` sont encore des stubs) sont **Phase 4** (voir `ROADMAP.md`).
...
- **Écritures fichiers (T1).** `fs.write` est limité en v0.1 à `/home/**` et `/var/home/**` (sous Fedora, `/home` est un lien vers `/var/home`) **et confiné au home de l'appelant** ; `fs.read`/`fs.list` de même (home + arbres système). La mémoire VibeOS n'est **pas** inscriptible via `fs.write` (deny codé en dur) : les écritures mémoire passent par **`memory.append`** (T1) — strictement additif, scopes `journal`, `knowledge`, `user` et `projects` (tous append-only ; la vue courante user/projects se lit via `vibectl memory profile`/`projects` — voir `docs/MEMORY.md` §9).
